...
from layers.RGCNBlock_v2 import RGCNModel
...
class ScaleGraphBlock(nn.Module):
    def __init__(self, configs):
        ...
        for i in range(self.k):
            self.gconv.append(
                RGCNModel(num_relations=configs.num_relations,
                          c_out=configs.c_out,
    ...
    def forward(self, x):
        ...
        for i in range(self.k):
            ...
            if self.seq_len % scale != 0:
                ...
                length = self.seq_len
                out = x
            out = out.reshape(B, length // scale, scale, d_model)

            # for Mul-attetion
            out = out.reshape(-1, scale, d_model)
            out = self.norm(self.att0(out))
            out = self.gelu(out)
            out = out.reshape(B, -1, scale, d_model).reshape(B, -1, d_model)
            # #for simpleVIT
        ...
        scale_weight = scale_weight.unsqueeze(1).unsqueeze(1).repeat(1, T, d_model, 1)
...
class Model(nn.Module):
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec, mask=None):
        ...
        # porject back
        dec_out = self.projection(enc_out)  # [B, T, d_model] -> [B, T, N]
        # dec_out = self.seq2pred(dec_out.transpose(1, 2)).transpose(1, 2)  # [B, T, N] -> [B, P, N]
