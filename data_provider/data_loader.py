...
class Dataset_ALFA(Dataset):
    def __init__(self, args, root_path, win_size, step=1, flag="train"):
        ...
        data = pd.read_csv(os.path.join(root_path, 'train.csv'))
        # 删掉训练集中的故障样本
        data = data.drop(data[(data[r'label'] == 1)].index, axis=0)
        # 对nan值进行填充
        ...
        if self.flag == "test":
            #  计算测试集标签中异常比例
            self.anomaly_ratio = np.sum(self.test_labels) / len(self.test_labels)
            print("anomaly_ratio:", self.anomaly_ratio)
            # self.anomaly_ratio_all = (np.sum(self.test_labels) + self.train.shape[0]) / len(self.test_labels)
            # print("anomaly_ratio_all:", self.anomaly_ratio_all)
